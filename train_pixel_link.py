...
from linkage import pixel_link
...
def config_initialization():
    ...
    proc_name = 'train_pixel_link_on'+ '_' + FLAGS.dataset_name
    try:
        import setproctitle
        setproctitle.setproctitle(proc_name)
    except:
        print("No module called setproctitle. Skip the proc_name modification.")
    ...
    with tf.device('/cpu:0'):
        ...
        image, glabel, gbboxes, gxs, gys = \
                ssd_vgg_preprocessing.preprocess_image(
                       ...
                       is_training = True, distorted=False)
...
def create_clones(batch_queue):        
    ...
    if update_ops:
        bn_update_op =  tf.group(*update_ops)
...
def train(train_op):
    ...
    init_fn = ckpt_util.get_init_fn(checkpoint_path = FLAGS.checkpoint_path, train_dir = FLAGS.train_dir, 
                          ignore_missing_vars = FLAGS.ignore_missing_vars, checkpoint_exclude_scopes = FLAGS.checkpoint_exclude_scopes)
    saver = tf.train.Saver(max_to_keep = 50, write_version = 2)
    slim.learning.train(
            train_op,

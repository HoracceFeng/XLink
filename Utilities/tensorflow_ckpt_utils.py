...
def get_iter(ckpt):
    iter_ = int(re.find_all('.ckpt-\d+', ckpt)[0].split('-')[-1])
    return iter_
...
def get_init_fn(checkpoint_path, train_dir, ignore_missing_vars = False,
    ...
    """
    if len(checkpoint_path)==0 or checkpoint_path is None: 
        return None
...
def min_area_rect(xs, ys):
    rects = tf.py_func(images.min_area_rect, [xs, ys], xs.dtype)
    ...
    for grad_and_vars in zip(*clone_grads):
        ...
        try:
            ...
            try:
                ...
                print("No module named pdb. python debugger not being used, skip it...")

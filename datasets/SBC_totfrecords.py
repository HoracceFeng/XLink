...
import config
import json
import complex_revert_four_points as crf
...
def cvt_to_tfrecords(output_path , data_path, gt_path):
    ...
    with tf.python_io.TFRecordWriter(output_path) as tfrecord_writer:
        for idx, image_name in enumerate(image_names):
            ...
            print " >>>>>> ", image_name, jsondict['imageWidth'], jsondict['imageHeight']
            ...
            for obj in jsondict['shapes']:
                ...
                if num_pts != 4:
                    ...
                    for i in range(len(gt)//2):
                        _oriented_box.append([int(gt[2*i]), int(gt[2*i+1])])
                    [x1,y1], [x2,y2], [x3,y3], [x4,y4] = crf.fourpoint(_oriented_box)
                    oriented_box = [x1,y1, x2,y2, x3,y3, x4,y4]
...
if __name__ == "__main__":
    ...
    ### SBC
    root_dir = util.io.get_absolute_path('/data/SBC/data-company')
    output_dir = util.io.get_absolute_path('/data/SBC/data-company/TFRecords/')
    _sets = 'train'
    ...
    cvt_to_tfrecords(output_path = util.io.join_path(output_dir, 'SBC_train.tfrecord'), data_path = training_data_dir, gt_path = training_gt_dir)

...
def resize_image_bboxes_with_crop_or_pad(image, bboxes, xs, ys,
                                         target_height, target_width, distorted=True):
    ...
    with tf.name_scope('resize_with_crop_or_pad'):
        ...
        if not distorted:
            ## for no crop trainin, now only support square target shape
            maxedge = max_(height, width)
            offset_prepad_width = max_((maxedge-width)//2, 0)
            offset_prepad_height = max_((maxedge-height)//2, 0)
            image = tf.image.pad_to_bounding_box(image, offset_prepad_height, offset_prepad_width,
                                                   maxedge, maxedge)
            bboxes, xs, ys = bboxes_crop_or_pad(bboxes, xs, ys,
                                        height, width,
                                        offset_prepad_height, offset_prepad_width,
                                        maxedge, maxedge)
            width = maxedge
            height = maxedge

...
def preprocess_for_train(image, labels, bboxes, xs, ys,
                         ...
                         scope='ssd_preprocessing_train', distorted=True):
...
#             image, bboxes, xs, ys = tf_image.random_rotate90(image, bboxes, xs, ys)
        ...
        if MAX_EXPAND_SCALE >= 1:  ## (2019.06.05 modify)
            ...
            def expand():
                ...
                return tf_image.resize_image_bboxes_with_crop_or_pad(
                             image, bboxes, xs, ys, target_h, target_w, distorted=distorted)
...
def preprocess_for_scale_eval(image, labels, bboxes, xs, ys,
                        data_format='NHWC',
                        scale=1,
                        scope='ssd_preprocessing_train'):
    """Preprocess an image for scale evaluation.
...
def preprocess_image(image,
    ...
    if is_training:
        return preprocess_for_train(image, labels, bboxes, xs, ys,
                                    out_shape=out_shape,
                                    data_format=data_format, distorted=distorted)
    else:

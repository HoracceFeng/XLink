...
# import util     ## pylib/src/util for decorator
...
def cal_gt_for_single_image(normed_xs, normed_ys, labels):
    ...
    for bbox_idx, (bbox_xs, bbox_ys) in enumerate(zip(xs, ys)):
        ...
        cv2,drawContours(bbox_mask, bbox_contours, idx = -1, color = 1, border_width = -1)        
    ...
    for bbox_idx, bbox_mask in enumerate(bbox_masks):
        ...
        cv2.drawContours(bbox_border_mask, new_bbox_contours, -1, color = 1, border_width = bbox_border_width * 2 + 1)
...
def rect_to_xys(rect, image_shape):
    ...
    # points = cv2.cv.BoxPoints(rect)       ## cv2.x
    points = cv2.boxPoints(rect)            ## cv3.x

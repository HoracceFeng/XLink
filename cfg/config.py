...
from Utilities import tensorflow_ckpt_utils as ckpt_util
from Utilities import looger
...
rotation_prob = 0.5
max_expand_scale = 1
expand_prob = 1                      # default 0, will not augment data
...
## default setting using num_channel=8*2, pixel fuse by addon method
## but pixel fuse by concat may have faster performance yet lower F-score (worth try)
## other type with "128" is actually num_channel=128=8*16, wired but can be seen as a try by author, can skip it
feat_fuse_type = pixel_link_symbol.FUSE_TYPE_cascade_conv1x1_upsample_sum
# feat_fuse_type = pixel_link_symbol.FUSE_TYPE_cascade_conv1x1_upsample_concat   ## (developing) may need an extra conv layer to fuse infos 
...
## feature map for pixel_link model (default use below, for 4s model)
...
def init_config(image_shape, batch_size = 1, 
    ...
    #init batch size
    global gpus
    gpus = ckpt_util.get_available_gpus(num_gpus)

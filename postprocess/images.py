...
def imwrite(path, img, rgb = False):
    ...
    if os.path.isdir(path):
        ...
        print("PathError: the provided path is not valid")
...
def get_contour_min_area_box(contour):
    ...
    # box = cv2.cv.BoxPoints(rect)    ## cv2.x
    box = cv2.boxPoints(rect)         ## cv3.x
    box = np.int0(box)
    return box
...
def get_contour_region_in_min_area_rect(img, cnt):
    ...
    # new_corners = util.dtype.int(np.transpose(new_corners))     ## pylib/src/dtype.py
